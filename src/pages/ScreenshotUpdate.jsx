...
function ScreenshotUpdatePage() {
  ...
  const historySheetName = "Screenshot Update"
  ...
  const fetchHistoryTasks = async () => {
    ...
    try {
      ...
      // Get columns A through D (first 4 columns)
      ...
      // Process each row (only columns A through D)
      const historyRows = []
      ...
      data.table.rows.forEach((row, rowIndex) => {
        ...
        // Process only columns A through D (first 4 columns)
        for (let i = 0; i < 4 && i < row.c.length; i++) {
          const cell = row.c[i]
  ...
  const processSheetData = (data) => {
    ...
    const cols = {
      ...
      delay: null,
      status: null,
      columnL: 51,            // Column L for filtering
      columnM: 52,            // Column M for filtering
    }
    ...
    data.table.cols.forEach((col, index) => {
      ...
      if (colLetter === "B") {
        ...
        cols.priority = index
      } else if (colLetter === "BB") {
        cols.pendingDays = index  // Using AP column for pending days like in QuotationValidationPage
    ...
    data.table.rows.forEach((row, rowIndex) => {
      ...
      const task = {
        ...
        pendingDays: pendingDays || days.toString(), // Use pendingDays from AP column if available, else use calculated days
                ...
                <tr key={task.id}>
                  {historyHeaders.map((header, index) => {
                    ...
                    if (value && (value.toLowerCase().includes('complete') || 
                                  ...
                                  value.toLowerCase().includes('uploaded'))) {
  ...
  return (
    <StageLayout
      ...
      description="Chat screenshot documentation"
      pendingContent={<PendingContent />}
      historyContent={<HistoryContent />}
      formLink="/forms/screenshot-update"
      formLinkText="Upload Screenshot"
    />
  )
}

export default ScreenshotUpdatePage