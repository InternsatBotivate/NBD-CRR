...
function QuotationValidationPage() {
  ...
  const historySheetName = "Quotation Validation"
  ...
  const fetchHistoryTasks = async () => {
    ...
    try {
      ...
      // Get columns A through K (first 11 columns)
      ...
      // Process each row (only columns A through K)
      const historyRows = []
      ...
      data.table.rows.forEach((row, rowIndex) => {
        ...
        // Process only columns A through K (first 11 columns)
        for (let i = 0; i < 11 && i < row.c.length; i++) {
          const cell = row.c[i]
  ...
  const processSheetData = (data) => {
    ...
    const cols = {
      ...
      quotationNumber: null,  // Column W 
      companyName: null,      // Column D
      quotationSharedBy: null, // Column Y
      contactPerson: null,    // Column E
      priority: null,         // Column J
      columnN: null,          // Column N for Pending
      delay: null,
      status: null,
      columnL: 39,            // Column L for filtering (index 11 is the 12th column, which is L)
      columnM: 40,            // Column M for filtering (index 12 is the 13th column, which is M)
    }

    data.table.cols.forEach((col, index) => {
      const label = col.label?.toLowerCase() || ""
      // Get the column letter using the new function
    ...
    data.table.rows.forEach((row, rowIndex) => {
      ...
      const task = {
        ...
        pending: getValue(cols.columnN), // Column N data for Pending column
        days: days,
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {pendingTasks.map((task) => (
                <tr key={task.id}>
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-sm">{task.pending}</td>
  ...
  return (
    <StageLayout
      ...
      description="Quotation validation and approvals"
      pendingContent={<PendingContent />}
      historyContent={<HistoryContent />}
      formLink="/forms/quotation-validation"
      formLinkText="Validate Quotation"
    />
  )
}

export default QuotationValidationPage