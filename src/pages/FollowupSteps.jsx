...
function FollowupStepsPage() {
  ...
  const historySheetName = "Followup Step"
  ...
  const fetchHistoryTasks = async () => {
    ...
    try {
      ...
      // Get columns A through H (first 8 columns)
      ...
      // Process each row (only columns A through H)
      const historyRows = []
      ...
      data.table.rows.forEach((row, rowIndex) => {
        ...
        // Process only columns A through H (first 8 columns)
        for (let i = 0; i < 8 && i < row.c.length; i++) {
          const cell = row.c[i]
  ...
  const processSheetData = (data) => {
    ...
    const cols = {
      ...
      priority: null,
      pendingDays: null,
      quotationSendStatus: null,
      quotationSharedBy: null,
      delay: null,
      status: null,
      columnL: 55,            // Column L for filtering
      columnM: 56,            // Column M for filtering
    }
    ...
    data.table.cols.forEach((col, index) => {
      ...
      if (colLetter === "B") {
        ...
        cols.priority = index
      } else if (colLetter === "AP") {
        cols.pendingDays = index
      } else if (colLetter === "AS") {
        cols.quotationSendStatus = index
      } else if (colLetter === "AI") {
        cols.quotationSharedBy = index
    ...
    data.table.rows.forEach((row, rowIndex) => {
      ...
      const task = {
        ...
        pendingDays: pendingDays || days.toString(),
        quotationSendStatus: getValue(cols.quotationSendStatus),
        quotationSharedBy: getValue(cols.quotationSharedBy),
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {pendingTasks.map((task) => (
                <tr key={task.id}>
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-sm">{task.quotationSendStatus}</td>
                  <td className="px-4 py-2 whitespace-nowrap text-sm">{task.quotationSharedBy}</td>
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-sm">{task.pendingDays}</td>
                  <td className="px-4 py-2 whitespace-nowrap text-sm">
                    <Link
                      ...
                      state={{
                        ...
                        quotationSendStatus: task.quotationSendStatus,
                        quotationSharedBy: task.quotationSharedBy,
                        priority: task.priority,
                        pendingDays: task.pendingDays
                      }}
                ...
                <tr key={task.id}>
                  {historyHeaders.map((header, index) => {
                    ...
                    if (value && (value.toLowerCase().includes('complete') || 
                                  ...
                                  value.toLowerCase().includes('interested') ||
                                  value.toLowerCase().includes('not interested') ||
                                  value.toLowerCase().includes('followup'))) {
                      ...
                      } else if (value.toLowerCase().includes('not interested')) {
  ...
  return (
    <StageLayout
      ...
      description="Followup planning and tracking"
      pendingContent={<PendingContent />}
      historyContent={<HistoryContent />}
      formLink="/forms/followup-steps"
      formLinkText="Record Followup"
    />
  )
}

export default FollowupStepsPage